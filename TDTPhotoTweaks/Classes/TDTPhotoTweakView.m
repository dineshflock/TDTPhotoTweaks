...
const CGFloat MaxRotationAngle = 0.5;
static const NSUInteger CropLines = 2;
static const NSUInteger GridLines = 9;

static const CGFloat CropViewHotArea = 16;
static const CGFloat MinimumCropArea = 60;
static const CGFloat MaximumCanvasWidthRatio = 0.9;
static const CGFloat MaximumCanvasHeightRatio = 0.7;
static const CGFloat CanvasHeaderHeigth = 60;
static const CGFloat CropViewCornerLength = 22;
...
@implementation TDTPhotoContentView
...
@interface TDTPhotoScrollView : UIScrollView

@property (nonatomic, strong) TDTPhotoContentView *photoContentView;

@end

@implementation TDTPhotoScrollView
...
- (instancetype)initWithCornerType:(CropCornerType)type {
  if (self = [super init]) {
    self.frame = CGRectMake(0, 0, CropViewCornerLength, CropViewCornerLength);
    self.backgroundColor = [UIColor clearColor];
    
    CGFloat lineWidth = 2;
    UIView *horizontal = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CropViewCornerLength, lineWidth)];
    horizontal.backgroundColor = [UIColor cropLineColor];
    [self addSubview:horizontal];
    
    UIView *vertical = [[UIView alloc] initWithFrame:CGRectMake(0, 0, lineWidth, CropViewCornerLength)];
    vertical.backgroundColor = [UIColor cropLineColor];
    [self addSubview:vertical];
    
    if (type == CropCornerTypeUpperLeft) {
      horizontal.center = CGPointMake(CropViewCornerLength / 2, lineWidth / 2);
      vertical.center = CGPointMake(lineWidth / 2, CropViewCornerLength / 2);
    } else if (type == CropCornerTypeUpperRight) {
      horizontal.center = CGPointMake(CropViewCornerLength / 2, lineWidth / 2);
      vertical.center = CGPointMake(CropViewCornerLength - lineWidth / 2, CropViewCornerLength / 2);
    } else if (type == CropCornerTypeLowerRight) {
      horizontal.center = CGPointMake(CropViewCornerLength / 2, CropViewCornerLength - lineWidth / 2);
      vertical.center = CGPointMake(CropViewCornerLength - lineWidth / 2, CropViewCornerLength / 2);
...
@property (nonatomic, weak) id<TDTCropViewDelegate> delegate;
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    _upperLeft.center = CGPointMake(CropViewCornerLength / 2, CropViewCornerLength / 2);
    ...
    _upperRight.center = CGPointMake(self.frame.size.width - CropViewCornerLength / 2, CropViewCornerLength / 2);
    ...
    _lowerRight.center = CGPointMake(self.frame.size.width - CropViewCornerLength / 2, self.frame.size.height - CropViewCornerLength / 2);
    ...
    _lowerLeft.center = CGPointMake(CropViewCornerLength / 2, self.frame.size.height - CropViewCornerLength / 2);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  if ([touches count] == 1 && self.isLocked == NO) {
    ...
    BOOL canChangeWidth = frame.size.width > MinimumCropArea;
    BOOL canChangeHeight = frame.size.height > MinimumCropArea;
    
    if (distanceBetweenPoints(location, p0) < CropViewHotArea) {
      if (canChangeWidth) {
    ...
    } else if (distanceBetweenPoints(location, p1) < CropViewHotArea) {
      if (canChangeWidth) {
        frame.size.width = location.x;
      }
      if (canChangeHeight) {
    ...
    } else if (distanceBetweenPoints(location, p2) < CropViewHotArea) {
      if (canChangeWidth) {
        frame.origin.x += location.x;
        frame.size.width -= location.x;
      }
    ...
    } else if (distanceBetweenPoints(location, p3) < CropViewHotArea) {
      if (canChangeWidth) {
        frame.size.width = location.x;
      }
      if (canChangeHeight) {
    ...
    } else if (fabs(location.x - p0.x) < CropViewHotArea) {
      if (canChangeWidth) {
        frame.origin.x += location.x;
        frame.size.width -= location.x;
      }
    } else if (fabs(location.x - p1.x) < CropViewHotArea) {
      if (canChangeWidth) {
        frame.size.width = location.x;
      }
    } else if (fabs(location.y - p0.y) < CropViewHotArea) {
      if (canChangeHeight) {
        frame.origin.y += location.y;
        frame.size.height -= location.y;
      }
    } else if (fabs(location.y - p2.y) < CropViewHotArea) {
      if (canChangeHeight) {
        frame.size.height = location.y;
      }
    }
...
@implementation TDTPhotoTweakView
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self = [super init]) {
    ...
    // scale the image
    _maximumCanvasSize = CGSizeMake(MaximumCanvasWidthRatio * self.frame.size.width,
                                    MaximumCanvasHeightRatio * self.frame.size.height - CanvasHeaderHeigth);
    ...
    _centerY = self.maximumCanvasSize.height / 2 + CanvasHeaderHeigth;
    
    _scrollView = [[TDTPhotoScrollView alloc] initWithFrame:bounds];
    ...
    _photoContentView = [[TDTPhotoContentView alloc] initWithImage:image];
    ...
    _cropView = [[TDTCropView alloc] initWithFrame:self.scrollView.frame];
